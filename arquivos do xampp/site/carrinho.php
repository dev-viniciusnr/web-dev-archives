...
<head>
    ...
    $sql4 = "SELECT * FROM cart WHERE iduser = '$idcliente'";

    $resultado_id = mysqli_query($link, $sql4);

    if($resultado_id){

        while($registro = mysqli_fetch_array($resultado_id, MYSQLI_ASSOC)){

            $item = $registro['iduser'];

        }}

    if($item == 0){
        header('Location: semcarrinho.php');
    }
...
<?php
    ...
    echo "<br><button class='btn btn-default'><a href='finalizar.php?valor=".$final_preco."'> FINALIZAR COMPRA</a></button>";
    ?>
